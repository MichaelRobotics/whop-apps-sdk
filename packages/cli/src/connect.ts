...
export function isConnectCommand(argv: string[]): boolean {
  const cmd = argv[0];
  return !cmd || cmd === "help" || cmd === "--help" || cmd === "-h" || CONNECT_COMMANDS.has(cmd);
}

export function gatewayBase(raw: string): string {
  return raw.replace(/\/+$/, "").replace(/\/mcp$/, "");
}

export function authorizeUrl(gateway: string, cliRedirect?: string): string {
  const base = `${gatewayBase(gateway)}/oauth/authorize`;
  if (!cliRedirect) return base;
  return `${base}?cli_redirect=${encodeURIComponent(cliRedirect)}`;
}

export function listenForSession(timeoutMs = 5 * 60 * 1000): Promise<{
  redirectUrl: string;
  ...
  return new Promise((resolve, reject) => {
    const server: Server = createServer((req, res) => {
      ...
      if (bearer) finish(bearer);
      else finish(undefined);
    });
    let finish = (_bearer: string | undefined) => {};
    const session = new Promise<string>((resolveSession, rejectSession) => {
      ...
      finish = (bearer) => {
        clearTimeout(timer);
        server.close();
        if (bearer) resolveSession(bearer);
        else rejectSession(new Error("Login redirect did not include a session."));
      };
    });
...
export function mcpEndpoint(gateway: string): string {
  return `${gatewayBase(gateway)}/mcp`;
}

export function parseBearer(input: string): string | undefined {
...
export function loadSession(home: string): SessionFile | undefined {
  const path = sessionPath(home);
  if (!existsSync(path)) return undefined;
  const parsed = JSON.parse(readFileSync(path, "utf8")) as Partial<SessionFile>;
  if (!parsed.gateway || !parsed.bearer || !parseBearer(parsed.bearer)) return undefined;
  return { gateway: gatewayBase(parsed.gateway), bearer: parseBearer(parsed.bearer)! };
}

export function saveSession(home: string, session: SessionFile): string {
  const path = sessionPath(home);
  mkdirSync(dirname(path), { recursive: true });
  const bearer = parseBearer(session.bearer);
  if (!bearer) throw new Error("Session bearer must look like sess_…");
...
function flags(argv: string[]): FlagMap {
...
function serverEntry(session: SessionFile, withType: boolean): McpServer {
...
function mergeMcpJson(path: string, session: SessionFile, withType: boolean): void {
...
function codexBlock(session: SessionFile): string {
...
function mergeCodex(path: string, session: SessionFile): void {
...
): Record<"cursor" | "claude" | "codex", string> {
  if (localDir) {
    return {
...
export function writeMcpConfig(
  home: string,
  session: SessionFile,
  clients: Array<"cursor" | "claude" | "codex">,
  localDir?: string,
): string[] {
  const targets = installTargets(home, localDir);
  const written: string[] = [];
  for (const client of clients) {
    if (client === "codex") mergeCodex(targets.codex, session);
    else mergeMcpJson(targets[client], session, client === "claude");
...
function jsonResult(status: number, body: unknown): RunResult {
  const text = `${JSON.stringify(body, null, 2)}\n`;
  if (status >= 400) return { code: 1, stdout: "", stderr: text };
  return { code: 0, stdout: text, stderr: "" };
}

function requireSession(home: string, urlOverride: string | undefined): SessionFile | RunResult {
  const saved = loadSession(home);
  if (!saved && !urlOverride) {
    return fail("No saved session. Run: whop-apps-gateway login");
  }
  if (!saved) return fail("No saved session bearer. Run: whop-apps-gateway login --bearer sess_…");
  return urlOverride ? { ...saved, gateway: gatewayBase(urlOverride) } : saved;
}

function parseClients(raw: string | undefined): Array<"cursor" | "claude" | "codex"> | string {
  const value = raw ?? "all";
  if (value === "all") return ["cursor", "claude", "codex"];
  ...
  return parts as Array<"cursor" | "claude" | "codex">;
}
...
export async function runConnect(argv: string[], io: ConnectIo = {}): Promise<RunResult> {
  ...
  if (cmd === "login") {
    ...
    if (given && !bearer) return fail("Bearer must look like sess_…");
    ...
    if (!bearer) {
      ...
      io.onAuthorize?.(url);
      if (!parsed.has("no-open")) open(url);
      try {
  ...
  if ("code" in sessionOrError) return sessionOrError;
  ...
  if (cmd === "connect") {
    ...
    if (!appId) return fail("connect requires --app-id app_xxx");
  ...
  if (cmd === "actions") {
    const appId = parsed.get("app-id");
    if (!appId) return fail("actions requires --app-id app_xxx");
  ...
  if (cmd === "invoke") {
    ...
    if (!appId || !action) return fail("invoke requires --app-id app_xxx --action name");
  ...
  if (cmd === "install") {
    ...
    if (typeof clients === "string") return fail(clients);
